...
- `editor_window.py` - Ventana principal del editor: atajos de teclado, historial de deshacer, diálogos de proyecto.
- `scene.py` - Escena del lienzo, arrastre de conexiones y gestión de eventos de elementos visuales.
- `view.py` - Lógica de vista gráfica, desplazamiento y zoom.
- `graph_items.py` - Elementos visuales de la escena: sockets, conexiones, renderizado de nodos y marcos.
- `command_nodes.py` - Clases de nodos Start y Command.
- `param_nodes.py` - Clases de nodos de parámetro (String, Bool, Integer, Float, Enum, Path, ...).
- `node_blueprint.py` - Especificaciones de nodos/sockets y constructores de definiciones de comandos y parámetros.
- `graph_serialization.py` - Formato único de instantánea para guardar/cargar/copiar-pegar/deshacer.
- `chain_execution.py` - Construye la invocación de la CLI de RealityCapture a partir del grafo de nodos.
- `command_database.py` - Carga el catálogo de comandos de RealityCapture ya analizado.
- `theme.py` - Colores derivados y hojas de estilo Qt para los widgets integrados.
- `color_picker.py` - Ventana emergente de selección de color HSVA.
- `inset_fill_checkbox.py` - Primitivo de casilla de verificación compartido.
- `flag_icon.py` - Icono de ventana que refleja el idioma de interfaz activo.
