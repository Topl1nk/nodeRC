...
- `editor_window.py` - Головне вікно редактора: гарячі клавіші, історія скасувань, діалоги проєкту.
- `scene.py` - Сцена полотна, перетягування з'єднань та обробка подій візуальних елементів.
- `view.py` - Логіка відображення графу, панорамування та масштабування.
- `graph_items.py` - Візуальні елементи сцени: сокети, з'єднання, відмальовування нод та рамок.
- `command_nodes.py` - Класи нод Start та Command.
- `param_nodes.py` - Класи параметричних нод (String, Bool, Integer, Float, Enum, Path тощо).
- `node_blueprint.py` - Специфікації нод/сокетів та побудова визначень команд і параметрів.
- `graph_serialization.py` - Єдиний формат знімків для збереження/завантаження/копіювання/скасувань.
- `chain_execution.py` - Побудова виклику RealityCapture CLI з графу нод.
- `command_database.py` - Завантаження розпарсеного каталогу команд RealityCapture.
- `theme.py` - Похідні кольори та стилі Qt для вбудованих віджетів.
- `color_picker.py` - Спливаюче вікно вибору кольору HSVA.
- `inset_fill_checkbox.py` - Спільний примітив чекбоксу.
- `flag_icon.py` - Іконка вікна, що відображає активну мову інтерфейсу.
