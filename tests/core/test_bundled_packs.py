...
def test_realitycapture_pack_manifest_is_discoverable():
    packs = discover_packs([BUNDLED_PACKS_DIR])

    assert [p.manifest.pack_id for p in packs] == ["realitycapture"]
    rc = packs[0]
    assert rc.manifest.display_name == "RealityCapture"
    assert rc.manifest.declares.executable is True
    assert rc.pack_dir == BUNDLED_PACKS_DIR / "realitycapture"


def test_realitycapture_pack_entry_point_exists():
    ...
    packs = discover_packs([BUNDLED_PACKS_DIR])
    rc = next(p for p in packs if p.manifest.pack_id == "realitycapture")

    assert (rc.pack_dir / rc.manifest.entry_point).is_file()


def test_realitycapture_pack_commands_source_matches_config():
    """pack.json's commands_source (relative — read generically by
    core/pack_catalog.py via pack_dir/commands_source) and
    packs/realitycapture/config.py's COMMAND_DB_JSON (absolute — read
