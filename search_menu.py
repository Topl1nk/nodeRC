...
from configuration import (
    ...
    SEARCH_DIALOG_HEIGHT, SEARCH_DIALOG_STYLESHEET
)
...
class SearchMenuDialog(QDialog):
    ...
    def _filter_tree(self, text):
        ...
        for item in self._all_items:
            ...
            if not item_payload: continue
            
            display_text = ""
            if "command" in item_payload:
                display_text = item_payload["display"].lower() + " " + item_payload.get("action_word", "").lower()
            elif "param_type" in item_payload:
                display_text = item_payload["display"].lower()
